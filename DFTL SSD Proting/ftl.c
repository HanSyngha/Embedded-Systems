...
#define META_BLKS_PER_BANK  (1 + 1) // include block #0, misc block
...
typedef struct _misc_metadata
{
    ...
    UINT32 cur_map_vpn; // phsical page for new map
...
UINT32 cache_miss, cache_hit;
UINT32 map_write_cnt;
UINT32 gc[NUM_BANKS] = { 0 }, map_gc;
...
static void   set_map_vcount(UINT32 const bank, UINT32 const vblock, UINT32 const vcount);
...
static void sanity_check(void)
{
    UINT32 dram_requirement = RD_BUF_BYTES + WR_BUF_BYTES + COPY_BUF_BYTES + FTL_BUF_BYTES
        + HIL_BUF_BYTES + TEMP_BUF_BYTES + BAD_BLK_BMP_BYTES + VCOUNT_BYTES + VCOUNT_MAP_BYTES + CMT_BYTES + GTD_BYTES + MAP_BUF_BYTES;
...
void ftl_read(UINT32 const lba, UINT32 const num_sectors)
{
    ...
    UINT32 bank, vpn, vpn_blk, vpn_page;
...
static void map_read(UINT32 const bank, UINT32 const gtd_idx, UINT32 const cache_slot)
{
    UINT32 gtd_vpn, gtd_blk, gtd_page;

    gtd_vpn = read_dram_32(gtd_idx_addr(bank, gtd_idx));

    write_dram_32(cmt_lpn_addr(bank, cache_slot), gtd_idx * (NUM_BANKS * N_MAP_ENTRIES_PER_PAGE));
    // If GTD is Not Assigned,
    if (gtd_vpn == NULL)
    {
        mem_set_dram(cmt_entry_addr(bank, cache_slot, 0), NULL, BYTES_PER_PAGE);
    }
    // If GTD is Assigned,
    else
    {
        extract_vpn(gtd_vpn, &gtd_blk, &gtd_page);
        flash_finish();
        nand_page_ptread(bank, gtd_blk, gtd_page, 0, SECTORS_PER_PAGE, MAP_BUF(bank), RETURN_WHEN_DONE);
        mem_copy(cmt_entry_addr(bank, cache_slot, 0), MAP_BUF(bank), BYTES_PER_PAGE);
    }
    write_dram_32(cmt_dirty_addr(bank, cache_slot), 0);
...
static void map_write(UINT32 const bank, UINT32 const gtd_idx, UINT32 const cache_slot)
{
    ...
    {
        extract_vpn(gtd_vpn, &gtd_blk, &gtd_page);
        set_map_vcount(bank, gtd_blk, get_map_vcount(bank, gtd_blk) - 1);
        set_gtd(bank, gtd_page, INVALID);
    }
    gtd_vpn  = assign_new_map_vpn(bank);
    extract_vpn(gtd_vpn, &gtd_blk, &gtd_page);
    set_gtd(bank, gtd_page, gtd_idx);
    set_map_vcount(bank, gtd_blk, get_map_vcount(bank, gtd_blk) + 1);
    write_dram_32(gtd_idx_addr(bank, gtd_idx), gtd_vpn);
    mem_copy(MAP_BUF(bank), cmt_entry_addr(bank, cache_slot, 0), BYTES_PER_PAGE);
    flash_finish();
    nand_page_ptprogram(bank, gtd_blk, gtd_page, 0, SECTORS_PER_PAGE, MAP_BUF(bank));

    mem_set_dram(cmt_entry_addr(bank, cache_slot, 0), NULL, BYTES_PER_PAGE);
    write_dram_32(cmt_lpn_addr(bank, cache_slot), 0);
    write_dram_32(cmt_dirty_addr(bank, cache_slot), 0);
    write_dram_32(cmt_time_addr(bank, cache_slot), 0);
...
static void set_map_vcount(UINT32 const bank, UINT32 const vblock, UINT32 const vcount)
{
    ...
    write_dram_16(VCOUNT_MAP_ADDR + (((bank * VBLKS_PER_BANK) + vblock) * sizeof(UINT16)), vcount);
}
static UINT32 assign_new_write_vpn(UINT32 const bank)
{
    ASSERT(bank < NUM_BANKS);
...
static UINT32 assign_new_map_vpn(UINT32 const bank)
{
    ...
    if ((write_vpn % PAGES_PER_BLK) == (PAGES_PER_BLK - 2))
    {
        mem_copy(MAP_BUF(bank), g_misc_meta[bank].gtd_list_of_cur_vblock, sizeof(UINT32) * PAGES_PER_BLK);
        // fix minor bug
        nand_page_ptprogram(bank, vblock, PAGES_PER_BLK - 1, 0,
                            ((sizeof(UINT32) * PAGES_PER_BLK + BYTES_PER_SECTOR - 1 ) / BYTES_PER_SECTOR), MAP_BUF(bank));
...
/*     uart_printf("garbage_collection bank %d, vblock %d",bank, vt_vblock); */
    ...
    // fix minor bug
    nand_page_ptread(bank, vt_vblock, PAGES_PER_BLK - 1, 0,
                     ((sizeof(UINT32) * PAGES_PER_BLK + BYTES_PER_SECTOR - 1 ) / BYTES_PER_SECTOR), MAP_BUF(bank), RETURN_WHEN_DONE);
    mem_copy(g_misc_meta[bank].gtd_list_of_cur_vblock, MAP_BUF(bank), sizeof(UINT32) * PAGES_PER_BLK);
    // 2. copy-back all valid pages to free space
    for (src_page = 0; src_page < (PAGES_PER_BLK - 1); src_page++)
    {
...
static UINT32 get_vt_vblock(UINT32 const bank)
{
    ...
    UINT32 vblock = META_BLKS_PER_BANK + 1;
    UINT32 min = VC_MAX;

    for (UINT32 blk = META_BLKS_PER_BANK + 1; blk < VBLKS_PER_BANK; blk++)
    {
        if (get_vcount(bank, blk) == VC_MAX)
            continue;
        if (get_vcount(bank, blk) < min)
        {
            vblock = blk;
            min = get_vcount(bank, blk);
        }
    }
...
static UINT32 get_vt_vblock_map(UINT32 const bank)
{
    ...
    for (UINT32 blk = META_BLKS_PER_BANK + 1; blk < VBLKS_PER_BANK; blk++)
    {
        if (get_map_vcount(bank, blk) == VC_MAX)
            continue;
        if (get_map_vcount(bank, blk) < min)
        {
            vblock = blk;
            min = get_map_vcount(bank, blk);
        }
    }

    ASSERT(is_bad_block(bank, vblock) == FALSE);
    ASSERT(vblock >= META_BLKS_PER_BANK && vblock < VBLKS_PER_BANK);
    ASSERT(get_map_vcount(bank, vblock) < (PAGES_PER_BLK - 1));
